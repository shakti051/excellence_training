...
class MyApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(home: Scaffold(body: ApiDatAcreen(),),);
  }
}
...
class ApiDatAcreen extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
   return ChangeNotifierProvider(create:(context)=>BackendService(),
     child: Consumer<BackendService>(
       builder: (context,data,child){
         if(data.getAppState==appSatte.INIT){
           ...
           return Center(child: Text(data.model.title.toString()),);
           }
         return CircularProgressIndicator();
         },)
